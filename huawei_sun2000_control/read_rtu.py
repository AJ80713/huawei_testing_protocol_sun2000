...
def connect_rtu(port, baudrate, bytesize, parity, stopbits, timeout):
    ...
    if client.connect():
        logging.info(
            f"Connected on {port} @ {baudrate}, {bytesize}-{parity}-{stopbits}"
        )
        return client
    else:
        logging.warning(
            (
                f"Failed to open {port} @ {baudrate}, "
                f"{bytesize}-{parity}-{stopbits}"
            )
        )
        return None
...
def main():
    # 1) Ensure the inverter is in RS-485/Modbus-RTU mode (not Wi-Fi).
    # 2) Wire A→485+, B→485− and connect GND to inverter ground.
    # 3) If there is no response, add a 120 Ω terminator across 485+/485−.
    #
    ...
    for bytesize, parity in [(8, "E"), (7, "E")]:
        ...
        logging.error(
            "All serial settings failed. Check inverter mode, wiring and "
            "COM port."
        )
...
if __name__ == "__main__":
    main()
# This script reads Modbus registers from an inverter via USB-RS485.
# It tries 8-E-1 then 7-E-1 and logs the results.
